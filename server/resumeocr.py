...
def de_bhai_pdf(dia_kya):
    ...
    if data['experience'] != None:
        ...
        for i in data['experience']:
            ...
            matches = datefinder.find_dates(i.replace('\xa0',' ').replace('-','gibbrish '))
            for match in matches:
                dates.append(match.strftime('%b %Y'))
            if len(dates)!=0:
                me=' '.join(i.split(' ')[:3])
                #u=finder.findall(i.replace('\xa0',' '))
                a={'Job title':'','Company name':me,'dates':dates,'Description':' '.join(i.replace('\xa0','').split(' ')[10:])}
                yay.append(a)
            dates=[] 

        regex = r"(?i)\b((?:https?://|www\d{0,3}[.]|[a-z0-9.\-]+[.][a-z]{2,4}/)(?:[^\s()<>]+|\(([^\s()<>]+|(\([^\s()<>]+\)))*\))+(?:\(([^\s()<>]+|(\([^\s()<>]+\)))*\)|[^\s`!()\[\]{};:'\".,<>?«»“”‘’]))" 
        data['Education']=''.join(lam[lam.upper().find('EDUCATION')+10: lam.upper().find('EDUCATION')+lam[lam.upper().find('EDUCATION')+12:].find('\n')])
        data['Links']=re.findall(regex,lam.replace('\n',''))
        data['experience']=yay
        yum=['college_name','degree','designation','company_names','no_of_pages']
        for i in yum:
            data.pop(i)
    else:        
        regex = r"(?i)\b((?:https?://|www\d{0,3}[.]|[a-z0-9.\-]+[.][a-z]{2,4}/)(?:[^\s()<>]+|\(([^\s()<>]+|(\([^\s()<>]+\)))*\))+(?:\(([^\s()<>]+|(\([^\s()<>]+\)))*\)|[^\s`!()\[\]{};:'\".,<>?«»“”‘’]))" 
